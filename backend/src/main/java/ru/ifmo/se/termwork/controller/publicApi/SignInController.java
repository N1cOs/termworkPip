...
public class SignInController {
    ...
    public ResponseEntity signIn(@RequestBody @Valid UserDto userDTO) {
        User user = userRepository.findByEmail(userDTO.getUsername()).
                orElseThrow(() -> new ApiException(HttpStatus.UNAUTHORIZED, InputErrors.Invalid.EMAIL));

        if(!passwordEncoder.matches(userDTO.getPassword(), user.getPassword()))
            throw new ApiException(HttpStatus.UNAUTHORIZED, InputErrors.Invalid.PASSWORD);
